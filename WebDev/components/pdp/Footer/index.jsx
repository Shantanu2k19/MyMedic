...
// import config from '../../next.config.js'  TODO check why used

const Footer = () => {
  const { theme, setTheme } = useTheme();
  const [visitors, setVisitors] = React.useState(-1);

  const getCount = async () => {
    try {
      const response = await fetch(`${config.serverUrl}/users/visitorCount`, {
        method: "GET",
